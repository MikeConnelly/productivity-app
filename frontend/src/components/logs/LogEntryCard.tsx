...
export function LogEntryCard({ log, entry, date }: LogEntryCardProps) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center gap-3 p-4 bg-white rounded-xl shadow-sm border-l-4 hover:shadow-md transition-shadow text-left"
      style={{ borderLeftColor: log.color }}
    >
      <span className="text-xl flex-shrink-0">{log.icon}</span>
      <div className="flex-1 min-w-0">
        <p className="font-medium text-gray-900 text-sm truncate">{log.name}</p>
        {entry?.content ? (
          <p className="text-xs text-gray-500 truncate mt-0.5">{entry.content}</p>
        ) : (
          <p className="text-xs text-gray-400 mt-0.5">Write entry...</p>
        )}
      </div>
    </button>
  );
