...
export const habitsApi = {
  ...
  complete: (habitId: string, date: string, note?: string) =>
    apiClient
      .post<Completion & { currentStreak: number; longestStreak: number }>(
  ...
  uncomplete: (habitId: string, date: string) =>
    apiClient
      .delete<{ currentStreak: number; longestStreak: number }>(
  ...
  getCompletionsRange: (from: string, to: string) =>
    apiClient
      .get<Completion[]>('/completions/range', { params: { from, to } })
      .then((r) => r.data),
};
